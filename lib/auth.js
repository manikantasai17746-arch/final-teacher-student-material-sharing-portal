...
if (!process.env.SESSION_SECRET || !String(process.env.SESSION_SECRET).trim()) {
  if (isProdAuth) {
    throw new Error(
      "SESSION_SECRET is required in production. Set SESSION_SECRET in Vercel Environment Variables " +
      "(or your host env). Do not rely on a random per-process secret."
    );
  }
  console.warn(
    "[eduvault] WARNING: SESSION_SECRET is not set. A temporary secret was generated for this " +
      "development process only; sessions will be invalidated on restart. Set SESSION_SECRET in .env."
  );
}
